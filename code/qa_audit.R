...
flagged_root_causes <- tribble(
  ...
  "ahrf_hospitals", "bracketed_zero_run", paste0(
    "NOT a parsing defect and NOT a coverage gap. The zeros are in the source, and the ",
    "edition-stacking mechanism that used to make them look worse has been FIXED upstream: AHRF ",
    "is now assembled by DATA year, taking the newest edition that covers each year, rather than ",
    "one point per edition (see area-health-resource-files/ingest.R, find_sas_vars). ",
    "That correction dated the Virginia reassignment properly. AHRF moved the Richmond hospitals ",
    "from Henrico County (51087) to Richmond City (51760) at data year 1992, and the two series ",
    "are now exact mirror images across that boundary -- Henrico runs 16-20 through 1991 and ",
    "Richmond City picks up 19 from 1992. The old series showed the break at 2001, which was an ",
    "artifact of which edition contributed which point, and stacked both editions' readings of ",
    "the same year as if they were different years. Impossible zero-runs on this measure fell ",
    "from 7 to 2 over the 1999-2025 window as a result. ",
    "What REMAINS is a source characteristic, not a pipeline defect: in AHRF's own latest view ",
    "Henrico reports 0 hospitals for 1992-2007 while Richmond City reports 9-19, so the ",
    "reassignment is real and the gap is faithfully represented. Do not try to repair it. ",
    "SEPARATELY, the wider ahrf_hospitals zero rate is not a defect either: 685 of 3,143 counties ",
    "(21.8%) report zero, against a published 691 of 3,142 (22.0%), the county values sum exactly ",
    "to the national total every year, and critical-access never exceeds total. Those zeros are ",
    "real rural counties."
  ),
