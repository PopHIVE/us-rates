...
# Census-native replacements for 14 measures historically sourced only from
# CHR's own redistribution -- same PEP/SAIPE/SAHIE direct sources as
# census_direct_long in populate_county_rates.R, now that those APIs'
# national ("00") geography is wired in. OQM's national row already existed
# in data_oqm.csv.gz (HRSA's own file ships a "00" total) but was never used
# at this level before. No chr_rural: census_ur_pct_urban_pop (its source)
# doesn't exist outside the county-level census file.
